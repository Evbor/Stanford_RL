...
class Linear(DQN):
    ...
    def add_placeholders_op(self):
        ...
        self.s = tf.placeholder(tf.uint8, shape=(None, *state_shape[:-1], state_shape[-1]*self.config.state_history))
        self.a = tf.placeholder(tf.int32, shape=(None,))
        self.r = tf.placeholder(tf.float32, shape=(None,))
        self.sp = tf.placeholder(tf.uint8, shape=(None, *state_shape[:-1], state_shape[-1]*self.config.state_history))
    ...
    def get_q_values_op(self, state, scope, reuse=False):
        ...
        out = tf.layers.dense(tf.layers.flatten(state), num_actions, name=scope, reuse=reuse)
    ...
    def add_update_target_op(self, q_scope, target_q_scope):
        ...
        for val in tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=q_scope):
            name = val.name.split('/')[-1].split(':')[0]
            shape = val.shape
            dtype = val.dtype
            
            with tf.variable_scope(target_q_scope, reuse=True) as scope:
                var = tf.get_variable(name, shape=shape, dtype=dtype)
                update_ops.append(tf.assign(var, val))
    ...
    def add_loss_op(self, q, target_q):
        ...
        q_a =tf.reduce_sum(q*tf.one_hot(self.a, num_actions), 1)
        self.loss = tf.reduce_mean(tf.squared_difference(q_samp, q_a))
    ...
    def add_optimizer_op(self, scope):
        ...
        grad_vars = adam.compute_gradients(loss=self.loss, var_list=tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=scope))
        grad_vars = [(tf.clip_by_norm(grad, self.config.clip_val), var) if self.config.grad_clip else (grad, var) for grad, var in grad_vars]
        
        self.train_op = adam.apply_gradients(grad_vars)
        self.grad_norm = tf.global_norm([grad for grad, var in grad_vars if grad is not None])
