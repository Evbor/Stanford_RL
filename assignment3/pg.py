...
def build_mlp(
    ...
    with tf.variable_scope(scope) as scope:
        ...
        for i in range(n_layers):
            cur_layer = tf.layers.Dense(size, activation=tf.nn.relu)(cur_layer)
        out = tf.layers.Dense(
                output_size,
...
class PG(object):
    ...
    def build_policy_network_op(self, scope="policy_network"):
        ...
        if self.discrete:
            ...
            self.sampled_action = tf.squeeze(
                    tf.random.categorical(action_logits, num_samples=1)
                    )
            ...
            std = tf.exp(log_std)
            self.sampled_action = (
                    tf.random.normal((self.config.batch_size, self.action_dim))
                    *std + action_means
                    )
            dist = tf.contrib.distributions.MultivariateNormalDiag(
                    loc=action_means, scale_diag=std
                    )
    ...
    def add_loss_op(self):
        ...
        self.loss = tf.reduce_mean(-1*self.logprob*self.advantage_placeholder)
        #######################################################
        #########          END YOUR CODE.          ############
    ...
    def add_baseline_op(self, scope="baseline"):
        ...
        self.baseline_target_placeholder = tf.placeholder(
                tf.float32, shape=(None,), name='baseline_target'
                )
        loss = tf.losses.mean_squared_error(
                self.baseline_target_placeholder, self.baseline
                )
        self.update_baseline_op = (
                tf.train.AdamOptimizer(learning_rate=self.lr).minimize(loss)
                )
        #######################################################
        #########          END YOUR CODE.          ############
    ...
    def get_returns(self, paths):
        ...
        for path in paths:
            ...
            #######################################################
            #########   YOUR CODE HERE - 5-10 lines.   ############
            returns = [] # this is [G_1,...,G_T] for the given path
            eplen = len(rewards)
            gammas = np.array([self.config.gamma**t for t in range(eplen)])
            for t in range(eplen):
                G_t = np.sum(rewards[t:]*gammas[:eplen-t])
                returns.append(G_t)
            #######################################################
            #########          END YOUR CODE.          ############
